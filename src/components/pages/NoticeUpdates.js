...
import '../../App.css';
import '../styles/NoticeUpdates.css'
...
function NoticeUpdates() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:4000/api/photos', formData, {
        headers: {
  ...
  return (
    ...
    <div className="board">
      ...
      <div className="notice-list">
        {notices.map((notice) => (
          <div key={notice._id}>
            <img style={{ width: 400, height: 400 }} src={`http://localhost:4000/uploads/${notice.image}`} alt="Notice" />
            <h3>{notice.notice}</h3>
          </div>
        ))}
      </div>
